...
dist/
├── chrome-extension/
└── WebSCADA_0.6.19_YYYYMMDD_HHMMSS.zip
```
...
- Kablo/havai çakışması belirsiz bırakılır; Hat MW/MVar hover'ları ortak kompakt zaman ve en uzun üç characteristic segment özeti kullanır.
- Hat (MVar) TM hover'ı staged gerilim değerini kendi zamanı ile, henüz yoksa `U —` olarak gösterir.

## v0.6.18

- Hat karakteristiklerinden güvenilir kablo oranı çıkarımı, kablo ağırlıklı (K) işareti ve Hat MW/MVar görünümünde hafif sarı kablo glow eklendi.
- Tooltip'ler karakteristik, uzunluk ve mevcutsa kablo uzunluğu/oranını gösterir.

## v0.6.17

- Hat (MVar) hover ve terminal-ok tooltip'leri, her iki terminalin Q/U değerlerini bağımsız zaman damgalarıyla gösterir; panel MVar değeri tam sayı, yön uyarısı ise ayrı satırdadır.
...
- Partial network background measurement IDs are retried first; unresolved IDs are retained only through a safe last-good merge, otherwise the result is rejected.
- Last-good live data remains visible during transient failures, then is marked stale after the refresh-aware threshold.
- Diagnostic writes are buffered, quota recovery is explicit, and the obsolete alarm background toggle has been removed.

## v0.6.13

- Tanılama logları compact request ID, sınırlı byte/ring alanı, batch yazım ve quota recovery ile güvenli saklanır; büyük geçici SCADA cache/snapshot verileri session alanına taşındı.
- Alarm runtime state’i bildirim ve sesten önce kaydedilir; bildirim başarısızsa bağlı ses atlanır, enabled kural izleme ise otomatik çalışır.
- Harita auto-refresh ağ/timeout hatalarında üç toplam deneme yapar ve sonraki üç dakikalık scheduler turunu korur.

## v0.6.12

- Background map sonuçları manuel V2 normalizer sözleşmesiyle doğrulanır; 0-match veya uyumsuz sonuç eski başarılı görünümü ezmez.
- Harita görünümü header-relative fixed viewport’a taşındı; sidebar kontrolü map alanında bağımsız konumlanır.

## v0.6.11

- Service-worker map sonuçları V2 harita state ve açık Şimşek listesine uygulanır; kapsam uyuşmazlıkları atlanıp tanılama loguna yazılır.
- Map, alarm, scheduler, Superset batch ve coordinator olayları kalıcı/redacted tanılama logunda; filtreli Log paneli ve CSV dışa aktarımında görünür.
- Kural kaydının ilk alarm kontrolü artık await edilir ve sonucu kullanıcıya bildirilir.

## v0.6.10

- Harita dikey alanı header sonrası kapsayıcıya bağlandı; panel ve sidebar düşük ekran yüksekliklerinde kendi alanında kalır.
- 3 dakikalık varsayılan yenileme, Chrome alarmının gerçek sonraki zamanı ve Hat Gerilim history metadata/fallback iyileştirildi.

## v0.6.9

- Gerilim metriği seçimi tüm modlarda görünür kalır; 66/154/400 kV Gerilim scope, harita ve panel aynı filtreyi kullanır.
- Büyük map sorgularında aynı fresh kapsam atlanır, kapsam değişince kalan batchler sınırda iptal edilir.

## v0.6.8

- Global ve sidebar tema geçişi tile katmanını aynı authoritative render yolundan yeniler.
- Harita SCADA sorguları cache uyumlu, coalesced ve hata sonrası son başarılı görünümü koruyacak şekilde güncellendi.

## v0.6.7

- Production SCADA mock yolu kaldırıldı; paket yalnız gerçek Chrome Extension/Superset aktarımını içerir.
- Individual alarm bildirimine 15 dakika susturma eklendi; alarm durumları ve harita durumu daha görünür hale getirildi.

## v0.6.6

- `PARTIAL_DATA` ve `PARTIAL_NETWORK` alarm sonucunu ayırır; no-current-data geçerli alternatif candidate'ları engellemez.
- İzlenen configured kapsam ile son değerlendirmedeki varlık sayısını alarm UI/CSV tanılarında ayrı gösterir.

## v0.6.5

- Alarm cache reuse süresi, manuel cadence izolasyonu, raw latest missing-ID recovery ve lease stale recovery iyileştirildi.
