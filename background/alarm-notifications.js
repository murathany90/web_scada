const WebSCADAAlarmNotifications = (() => {
  ...
  async function alarm(rule, entry) { const id = `webscada-alarm:${rule.id}:${entry.entityId}`; return notify(id, { title: `WebSCADA — ${rule.severity === 'warning' ? 'Uyarı' : 'Kritik alarm'}`, message: `${entry.entityDisplayName || entry.entityName}\nYüklenme %${entry.loadingPct.toFixed(1)} · Limit %${rule.thresholdPct}` }); }
  ...
  return { diagnostic, sound, activeBadge, alarm, summary, notify };
})();
