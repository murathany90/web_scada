const WebSCADAMapRefresh = (() => {
  ...
  async function run(trigger = 'map-background', wake = {}) {
    ...
    try {
      ...
      let { result, attempts } = await executeWithRetry(payload, context, trigger); result = await retryPartialNetwork(result, payload, context, trigger); const completedAt = new Date().toISOString(); const rows = resultRows(result); const meta = result.meta || {}; const durationMs = Date.now() - new Date(startedAt).getTime(); const resultEntry = { at: Date.now(), startedAt, completedAt, durationMs, returnedRows: rows, scope: context.scope, data: result.data, transport: { authMode: result.authMode, httpStatus: result.httpStatus, usedFallback: Boolean(result.usedFallback), meta } };
