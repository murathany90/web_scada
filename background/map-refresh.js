const WebSCADAMapRefresh = (() => {
  ...
  async function run(trigger = 'map-background', wake = {}) {
    ...
    try {
      ...
      await storeStatus({ enabled: true, running: false, lastStartedAt: startedAt, lastCompletedAt: completedAt, lastTrigger: trigger, lastDurationMs: Date.now() - new Date(startedAt).getTime(), lastError: '', lastErrorType: '' }, settings.autoRefreshMinutes);
      return { ok: true, result: resultEntry };
    } catch (error) {
      const completedAt = new Date().toISOString(); const type = error.type || errorType(error);
      await storeStatus({ ...old, enabled: true, running: false, lastStartedAt: startedAt, lastCompletedAt: completedAt, lastTrigger: trigger, lastError: error.message || String(error), lastErrorType: type }, settings.autoRefreshMinutes);
