((root, factory) => { const api = factory(); if (typeof module === 'object' && module.exports) module.exports = api; root.WebSCADASettings = api; })(typeof globalThis !== 'undefined' ? globalThis : this, () => {
  ...
  const DEFAULTS = Object.freeze({ autoRefreshEnabled: true, autoRefreshMinutes: 3, capacitySeason: 'winter', reactiveReference154Mvar: 120, reactiveReference400Mvar: 300, alarmNotificationsEnabled: true, alarmSoundEnabled: true, warningSound: 'warning_02_double_beep.wav', criticalSound: 'critical_01_dualtone.wav', alarmVolume: 100 });
  const minutes = value => [1, 2, 3, 5, 10, 15].includes(Number(value)) ? Number(value) : DEFAULTS.autoRefreshMinutes;
  function normalize(input = {}, legacy = {}) {
    const source = { ...DEFAULTS, ...legacy, ...input };
    const reference = (value, fallback) => Number.isFinite(Number(value)) && Number(value) > 0 && Number(value) <= 10000 ? Number(value) : fallback;
    return { ...source, autoRefreshEnabled: source.autoRefreshEnabled !== false, autoRefreshMinutes: minutes(source.autoRefreshMinutes), capacitySeason: source.capacitySeason === 'summer' ? 'summer' : 'winter', reactiveReference154Mvar: reference(source.reactiveReference154Mvar, DEFAULTS.reactiveReference154Mvar), reactiveReference400Mvar: reference(source.reactiveReference400Mvar, DEFAULTS.reactiveReference400Mvar), alarmNotificationsEnabled: source.alarmNotificationsEnabled !== false, alarmSoundEnabled: source.alarmSoundEnabled !== false, warningSound: WARNING_SOUNDS.includes(source.warningSound) ? source.warningSound : DEFAULTS.warningSound, criticalSound: CRITICAL_SOUNDS.includes(source.criticalSound) ? source.criticalSound : DEFAULTS.criticalSound, alarmVolume: Math.max(0, Math.min(100, Number.isFinite(Number(source.alarmVolume)) ? Number(source.alarmVolume) : DEFAULTS.alarmVolume)) };
  }
