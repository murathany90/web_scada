...
function updateSharedMapStatus() {
  ...
  else if (scada.enabled && scada.lastDataTimestamp) { text = `CANLI · Veri ${clock(scada.lastDataTimestamp)}${next} · ${auto}${enrichmentActive ? ` · Eksikler tamamlanıyor %${Math.round(Number(op.progressPct) || 0)}` : ` · Son ${clock(fetch.finishedAt || scada.lastFetchAt)} · TAMAM`}`; level = 'success'; }
