...
function updateSharedMapStatus() {
  const scada = state.scada || {}; const fetch = scada.fetchMeta || {}; const op = scada.operationMeta || {}; const scheduler = scada.backgroundRefreshStatus || {}; const clock = value => value ? new Date(value).toLocaleTimeString('tr-TR') : '—'; const countdown = value => { const ms = new Date(value).getTime() - Date.now(); if (!Number.isFinite(ms) || ms < 0) return ''; const seconds = Math.floor(ms / 1000); return `${String(Math.floor(seconds / 60)).padStart(2, '0')}:${String(seconds % 60).padStart(2, '0')}`; }; const auto = scada.autoRefresh ? `Oto ${scada.autoRefreshMinutes || 3} dk` : 'Oto kapalı'; const next = scheduler.nextScheduledAt ? ` · Sonraki ${clock(scheduler.nextScheduledAt)}${countdown(scheduler.nextScheduledAt) ? ` · ${countdown(scheduler.nextScheduledAt)}` : ''}` : ''; const backgroundResult = scheduler.lastCompletedAt ? ` · BG ${scheduler.result || 'OK'}${Number.isFinite(Number(scheduler.returnedRows)) ? ` ${scheduler.returnedRows} satır` : ''}` : ''; const enrichmentActive = op.kind === 'enrichment' && !['done', 'error', 'discarded', 'cancelled'].includes(op.stage); let text = state.map.status?.text || 'Hazır'; let level = state.map.status?.level || 'info';
  ...
  else if (!enrichmentActive && (fetch.status === 'loading' || ['running', 'batches', 'queued'].includes(op.stage))) { text = op.stage === 'queued' || fetch.stage === 'queued' ? 'BEKLİYOR · Superset kuyruğu' : `SORGULANIYOR · %${Math.round(Number(op.progressPct ?? fetch.progressPct) || 0)} · ${scada.currentScope?.modeLabel || 'SCADA'}`; level = 'info'; }
  else if (fetch.status === 'error' || scada.error) { const hasSnapshot = scada.entityMetricsByKey instanceof Map && scada.entityMetricsByKey.size && (scada.lastDataTimestamp || scada.lastFetchAt); text = hasSnapshot ? `UYARI · Yeni veri alınamadı · son başarılı veri ${clock(scada.lastDataTimestamp || scada.lastFetchAt)} gösteriliyor` : `HATA · ${fetch.errorType || scada.errorType || fetch.error || scada.error || 'sorgu'} · Son başarılı veri ${clock(scada.lastDataTimestamp)}`; level = hasSnapshot ? 'info' : 'error'; }
  else if (scada.enabled && scada.lastDataTimestamp) { text = `CANLI · Veri ${clock(scada.lastDataTimestamp)}${next}${backgroundResult} · ${auto}${enrichmentActive ? ` · Eksikler tamamlanıyor %${Math.round(Number(op.progressPct) || 0)}` : ` · Son ${clock(fetch.finishedAt || scada.lastFetchAt)} · TAMAM`}`; level = 'success'; }
