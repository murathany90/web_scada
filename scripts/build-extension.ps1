...
$stamp = Get-Date -Format 'yyyyMMdd_HHmmss'; $zip = Join-Path $dist "WebSCADA_0.5.0_$stamp.zip"
...
try {
  ...
  foreach ($required in @('manifest.json','app.html','app.js','offscreen/alarm-audio.html','offscreen/alarm-audio.js','data/kml_layers_v2.json','data/mapping.json','data/scada_auth.json')) { if ($entryNames -notcontains $required) { throw "Paket eksik runtime dosyasi: $required" } }
