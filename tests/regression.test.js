...
test('partial workspace results keep successful rows and expose an explicit UI warning', () => { assert.match(source('background/query-service.js'), /results\.push\(await chartFirst/); assert.match(source('app.js'), /Kısmi veri:/); assert.match(source('app.js'), /failedBatches/); });
