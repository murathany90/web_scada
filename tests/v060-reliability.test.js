...
test('scheduler self-heals stale alarms and a not-due wake finalizes status', async () => {
  ...
  global.WebSCADAAlarmModel = require('../core/alarm-model.js'); global.WebSCADAAlarmCatalog = catalog; delete require.cache[require.resolve('../background/alarm-monitor.js')]; const monitor = require('../background/alarm-monitor.js'); const status = await monitor.finalizeCycleStatus({ running: true }, Date.now(), 'alarm-background', { skipped: 'not-due' }); assert.equal(status.running, false); assert.equal(status.lastSkippedReason, 'not-due'); assert.ok(status.lastWakeAt);
